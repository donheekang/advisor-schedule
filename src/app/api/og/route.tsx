    ...
    (
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ fontSize: 24, color: '#ff9b5e' }}>{category}</div>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div style={{ fontSize: 58, fontWeight: 800, lineHeight: 1.25 }}>{title}</div>
          <div style={{ fontSize: 28, color: '#ff7a45' }}>전국 실데이터 기반 반려동물 진료비 비교</div>
        </div>

        <div style={{ fontSize: 20, color: '#ff9b5e' }}>pethealthplus.kr</div>
      </div>
    ),
    {
      width: 1200,
