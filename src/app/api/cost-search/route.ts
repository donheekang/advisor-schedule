...
type CostSearchApiResponse = {
  results: CostSearchItem[];
  total: number;
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(
      `${RENDER_API_BASE}/api/cost/search?q=${encodeURIComponent(q)}&species=${species}&region=${encodeURIComponent(region)}`,
    ...
    if (res.ok) {
      const data = (await res.json()) as CostSearchApiResponse;
  ...
  const filtered = COST_SEARCH_SEED_DATA.filter((item) => {
    ...
    const keywordMatch =
      !normalizedQ ||
      item.item.toLowerCase().includes(normalizedQ) ||
      item.category.toLowerCase().includes(normalizedQ);
    return speciesMatch && keywordMatch;
  });

  const seedResults: CostSearchItem[] = filtered.slice(0, 20).map((item) => ({
    item: item.item,
