      ...
      <div className="mx-auto w-full max-w-4xl">
        <section className="py-12 md:py-16">
          <AnimateOnScroll animation="fade-up">
            <span className="mb-4 inline-flex items-center gap-1.5 rounded-full bg-[#FFF3E6] px-3 py-1.5 text-xs font-semibold text-[#F97316]">30초 무료 AI 진료비 분석</span>
            <h1 className="mb-3 text-2xl font-bold tracking-tight text-[#17191f] md:text-4xl">우리 아이 맞춤 케어,<br />지금 바로 확인하세요</h1>
            <p className="text-sm text-[#ff7a45]">로그인 없이 간단한 정보만 입력하면 맞춤 관리 포인트와 케어 추천을 보여드려요.</p>
          </AnimateOnScroll>
        </section>

        <form onSubmit={handleSubmit} className="space-y-6">
          <AnimateOnScroll animation="fade-up" delay={100}>
            <div className="rounded-2xl border border-[#fff0ea]/10 bg-white p-6 shadow-sm md:p-8">
              <h2 className="mb-6 text-lg font-bold text-[#17191f]">STEP 1. 아이 정보를 입력해요</h2>

              <div className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-[#ff7a45]">종류</label>
                  <select
                    value={form.species}
                    onChange={(event) => setForm((prev) => ({ ...prev, species: event.target.value as FormState['species'] }))}
                    className="w-full rounded-xl border border-[#fff0ea] bg-white px-4 py-3 text-sm text-[#17191f] outline-none transition-all duration-200 focus:border-[#F97316] focus:ring-2 focus:ring-[#F97316]/20"
                  >
                    <option>강아지</option>
                    <option>고양이</option>
                  </select>
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-[#ff7a45]">품종 선택</label>
                  <select
                    value={form.breed}
              ...
              <div className="mb-6">
                <label className="mb-3 block text-sm font-semibold text-[#17191f]">최근 진료 이력</label>
            ...
            <div className="mx-auto mt-8 max-w-2xl">
              <div className="relative overflow-hidden rounded-3xl border border-[#0B3041]/[0.06] bg-white p-6 shadow-sm">
                ...
                <div className="opacity-80">
                  <p className="mb-1 text-xs font-semibold text-[#48B8D0]">AI 진료비 · 예시</p>
          ...
          <AnimateOnScroll animation="fade-up">
            <section ref={resultRef} className="mt-6 rounded-2xl border border-[#fff0ea]/10 bg-white p-6 shadow-sm md:p-8">
              <h2 className="mb-2 text-lg font-bold text-[#17191f]">STEP 2. 맞춤 분석 결과</h2>
              ...
              {isLoading ? (
                ...
                <div className="py-12 text-center">
                  <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-[#FFF3E6]">
                    <svg className="h-8 w-8 text-[#F97316]/50" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="m12 3 1.8 5.2L19 10l-5.2 1.8L12 17l-1.8-5.2L5 10l5.2-1.8L12 3Z" /></svg>
                  </div>
                  <p className="text-sm text-[#ff7a45]">입력을 완료하고 분석 버튼을 누르면 결과가 여기에 표시돼요.</p>
                </div>
              ) : (
                <div className="space-y-4">
                  <p className="text-sm text-[#17191f]">{report.summary}</p>
                  {aiInsight ? <p className="whitespace-pre-line rounded-xl bg-[#FFF8F0] p-4 text-sm text-[#17191f]">{aiInsight}</p> : null}
                  {remaining !== null ? <p className="text-xs text-[#ff7a45]">오늘 남은 AI 인사이트: {remaining}회</p> : null}
                  <button type="button" onClick={() => void handleCopy()} className="rounded-xl border border-[#fff0ea] px-4 py-2 text-sm font-medium text-[#17191f]">결과 복사하기</button>
                </div>
              )}
            </section>
          </AnimateOnScroll>
