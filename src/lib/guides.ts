...
const renderMarkdownToHtml = (markdown: string): string => {
  ...
  lines.forEach((line) => {
    ...
    if (trimmed.startsWith('## ')) {
      ...
      html.push(`<h2 class="mt-8 text-2xl font-bold text-[#17191f]">${renderInlineMarkdown(trimmed.slice(3))}</h2>`);
      return;
    }
    ...
    if (trimmed.startsWith('### ')) {
      ...
      html.push(`<h3 class="mt-6 text-xl font-bold text-[#17191f]">${renderInlineMarkdown(trimmed.slice(4))}</h3>`);
      return;
    }
