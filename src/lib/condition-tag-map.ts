...
export const BREED_RISK_TAGS: BreedProfile[] = [
  { breed: '말티즈', species: 'dog', intro: '눈물, 치아, 슬개골 관리가 중요한 소형견이에요.', riskTags: [
  ...
  { breed: '푸들', species: 'dog', intro: '귀·피부와 치아 관리 수요가 많은 품종이에요.', riskTags: [
  ...
  { breed: '포메라니안', species: 'dog', intro: '기관·심장·치과 관련 관리가 자주 언급되는 품종이에요.', riskTags: [
  ...
  { breed: '치와와', species: 'dog', intro: '치과·관절·저체온 관리가 중요한 초소형견이에요.', riskTags: [
  ...
  { breed: '비숑프리제', species: 'dog', intro: '피부·눈·치과 관리 수요가 꾸준한 품종이에요.', riskTags: [
  ...
  { breed: '골든리트리버', species: 'dog', intro: '대형견 특성상 관절·종양·소화기 검진이 중요해요.', riskTags: [
  ...
  { breed: '래브라도리트리버', species: 'dog', intro: '비만·관절·피부 질환 관리가 중요한 대형견이에요.', riskTags: [
  ...
  { breed: '웰시코기', species: 'dog', intro: '허리·관절·체중 관리가 핵심인 품종이에요.', riskTags: [
  ...
  { breed: '시바이누', species: 'dog', intro: '피부·알레르기·소화기 체크가 자주 필요한 견종이에요.', riskTags: [
  ...
  { breed: '프렌치불도그', species: 'dog', intro: '호흡기·피부·척추 관련 진료가 많은 견종이에요.', riskTags: [
  ...
  { breed: '요크셔테리어', species: 'dog', intro: '치아·기관·간 기능 관련 관리가 자주 필요해요.', riskTags: [
  ...
  { breed: '닥스훈트', species: 'dog', intro: '디스크·관절 관리가 특히 중요한 견종이에요.', riskTags: [
  ...
  { breed: '코리안숏헤어', species: 'cat', intro: '소화기·치과·비뇨기 체크가 자주 필요한 고양이예요.', riskTags: [
  ...
  { breed: '브리티시숏헤어', species: 'cat', intro: '비만·치과·심장 검진을 꾸준히 보는 편이에요.', riskTags: [
  ...
  { breed: '러시안블루', species: 'cat', intro: '스트레스성 질환과 구강 관리에 신경 쓰는 품종이에요.', riskTags: [
  ...
  { breed: '스코티시폴드', species: 'cat', intro: '관절·연골 관련 통증 관리가 중요한 품종이에요.', riskTags: [
  ...
  { breed: '노르웨이숲', species: 'cat', intro: '피모·심장·체중 관리가 핵심인 장모종이에요.', riskTags: [
  ...
  { breed: '메인쿤', species: 'cat', intro: '심장·관절·체중 관련 모니터링이 중요한 대형묘예요.', riskTags: [
  ...
  { breed: '샴', species: 'cat', intro: '호흡기·치과·소화기 관리가 자주 언급되는 품종이에요.', riskTags: [
  ...
  { breed: '페르시안', species: 'cat', intro: '안과·호흡기·피부 관련 관리 수요가 높은 품종이에요.', riskTags: [
  ...
  { breed: '벵갈', species: 'cat', intro: '활동량이 높아 근골격·소화기 관리가 중요한 품종이에요.', riskTags: [
...
export function getPopularBreeds() {
  return BREED_RISK_TAGS.map((item) => item.breed);
}

export function getBreedProfile(breed: string) {
  return BREED_RISK_TAGS.find((item) => item.breed === breed);
}

export function getCategoryAverageCost(categorySlug: BreedRiskItem['category'], species: 'dog' | 'cat') {
  ...
  const matched = COST_SEARCH_SEED_DATA.filter((item) => {
    ...
    if (categorySlug === 'dental') return item.category.includes('치과') || item.item.includes('스케일링') || item.item.includes('발치');
    if (categorySlug === 'surgery') return item.category.includes('수술') || item.item.includes('수술') || item.item.includes('중성화');
    if (categorySlug === 'vaccine') return item.category.includes('예방') || item.item.includes('예방접종');
    if (categorySlug === 'lab') return item.category.includes('검사') || item.item.includes('검사') || item.item.includes('건강검진');
    if (categorySlug === 'imaging') return item.category.includes('영상') || item.item.includes('초음파') || item.item.includes('MRI') || item.item.includes('CT');
    if (categorySlug === 'medication') return item.item.includes('약') || item.item.includes('소염');
