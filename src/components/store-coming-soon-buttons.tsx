...
export function StoreComingSoonButtons({ tone = 'light', className }: StoreButtonsProps) {
  const buttonClassName =
    tone === 'dark'
      ? 'flex flex-1 items-center justify-center gap-2 rounded-2xl bg-white/10 px-5 py-4 text-sm font-semibold text-white transition-all hover:bg-white/20'
      : 'flex flex-1 items-center justify-center gap-2 rounded-2xl bg-[#0B3041]/5 px-5 py-4 text-sm font-semibold text-[#0B3041]/70 transition-all hover:bg-[#0B3041]/10';

  return (
    <div className={'flex w-full gap-3 ' + (className ?? '')}>
      <a href={APP_STORE_URL} target="_blank" rel="noopener noreferrer" className={buttonClassName}>
      ...
      <a href={PLAY_STORE_URL} target="_blank" rel="noopener noreferrer" className={buttonClassName}>
