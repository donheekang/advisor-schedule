      ...
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-6 px-4 pb-20 pt-24 md:pt-28">
        ...
        {!hasSearched ? (
          <article className="rounded-3xl bg-white p-10 text-center shadow-lg ring-1 ring-[#F8C79F]/20">
            ...
            <p className="mt-2 text-sm text-[#7C4A2D]">전국 평균과 비교해서 적정 가격인지 확인할 수 있어요</p>
          </article>
        ) : null}
        ...
        {searchError && !costResult ? (
          <article className="rounded-3xl bg-white p-10 text-center shadow-lg ring-1 ring-[#F8C79F]/20">
            <p className="text-5xl">😢</p>
            <p className="mt-4 text-lg font-bold text-[#4F2A1D]">해당 항목의 데이터가 아직 없어요</p>
          ...
          <article className="space-y-5 rounded-3xl bg-white p-6 shadow-lg ring-1 ring-[#F8C79F]/20">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              <div className="rounded-2xl bg-gradient-to-b from-[#FFF8F0] to-[#FFEDD5] p-5 ring-1 ring-[#F8C79F]/30">
                ...
                <p className="mt-2 text-2xl font-extrabold text-[#F97316]">{toWon(costResult.priceStats.avg)}</p>
              </div>
              <div className="rounded-2xl bg-gradient-to-b from-[#FFF8F0] to-[#FFEDD5] p-5 ring-1 ring-[#F8C79F]/30">
                <p className="text-xs font-semibold text-[#A36241]">전국 평균</p>
                <p className="mt-2 text-2xl font-extrabold text-[#F97316]">{toWon(costResult.nationalAvg)}</p>
              </div>
              <div className="rounded-2xl bg-gradient-to-b from-[#FFF8F0] to-[#FFEDD5] p-5 ring-1 ring-[#F8C79F]/30">
                <p className="text-xs font-semibold text-[#A36241]">{region} 평균</p>
                <p className="mt-2 text-2xl font-extrabold text-[#F97316]">{toWon(costResult.regionalAvg)}</p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {costResult.sources.map((source) => (
                <span key={source} className="rounded-full bg-[#FFF8F0] px-3 py-1.5 text-xs font-medium text-[#A36241] ring-1 ring-[#F8C79F]/30">
                  {source}
                </span>
              ))}
            </div>
              ...
              <div className="mt-2 rounded-2xl bg-[#FFF8F0] p-5 ring-1 ring-[#F8C79F]/20">
                ...
                {myComparison ? (
                  <p className="mt-3 text-sm text-[#4F2A1D]">
                    내 평균은 <span className="font-extrabold text-[#F97316]">{toWon(myComparison.average)}</span>이며, 전체 평균보다{' '}
                    <span className="font-extrabold text-[#F97316]">{Math.abs(myComparison.diffPercent).toFixed(1)}%</span>{' '}
                    {myComparison.isHigher ? '높아요.' : '낮아요.'}
                  </p>
                ) : (
                  <p className="mt-3 text-xs text-[#A36241]">로그인 기록 기반으로 같은 항목을 비교합니다.</p>
                )}
              </div>
            ) : (
              <p className="text-xs text-[#A36241]">로그인하면 내 기록과의 비교도 확인할 수 있어요.</p>
            )}
          </article>
        ) : null}
          ...
          <article className="rounded-3xl bg-white p-6 shadow-lg ring-1 ring-[#F8C79F]/20">
            ...
            {loading ? (
              ...
              <div className="mt-4 space-y-3 rounded-2xl bg-[#FFF8F0] p-5 ring-1 ring-[#F8C79F]/20">
                ...
                <p className="text-xs text-[#A36241]">※ 의료적 판단은 제공하지 않으며, 가격 비교 및 항목 설명 중심으로 안내해요.</p>
              </div>
            ) : !isPremiumUser ? (
              <div className="mt-4">
                <Paywall
